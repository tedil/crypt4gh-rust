...
pub fn encrypt<R: Read, W: Write>(
    ...
    rnd.try_fill_bytes(&mut session_key)
...
pub fn encrypt_header(
    ...
    let session_key_or_new = session_key.map_or_else(
        || {
            ...
            rnd.try_fill_bytes(&mut session_key)
                .map_err(|_| Crypt4GHError::NoRandomNonce)?; // TODO: Custom error for this
...
pub fn decrypt<R: Read, W: Write>(
    ...
    match edit_list {
        ...
        Some(edit_list_content) => {
            body_decrypt_parts(read_buffer, session_keys, write_info, edit_list_content)?
        }
    }
    ...
    fn skip(&mut self, size: usize) -> Result<(), Crypt4GHError> {
        ...
        while remaining_size > 0 {
            ...
            if remaining_size >= SEGMENT_SIZE {
                ...
                log::debug!(
                    "skip()'s skipping a whole segment, remaining size: {}",
                    remaining_size
                );
            } else {
                ...
                log::debug!(
                    "skip()'s Index for remaining_size of the segment = {}",
                    self.index
                );
...
pub fn body_decrypt_parts<W: Write>(
    ...
    for edit_length in edit_list {
        if skip {
            if edit_length != 0 {
                log::debug!(
                    "body_decrypt_parts()'s edit_length from edit list: {}",
                    edit_length
                );
...
fn decrypt_block(ciphersegment: &[u8], session_keys: &[Vec<u8>]) -> Result<Vec<u8>, Crypt4GHError> {
    ...
    session_keys
        ...
        .ok_or(Crypt4GHError::UnableToDecryptBlock(
            ciphersegment.to_vec(),
            "error decrypting block".to_string(),
        ))
}
...
pub fn rearrange<R: Read, W: Write>(
    ...
    let (packets, mut segment_oracle) =
        header::rearrange(header_packets, keys, range_start, range_span, &None)?;
    write_buffer.write_all(&header::serialize(packets))?;
