...
use chacha20poly1305::{self, aead, AeadCore, ChaCha20Poly1305, KeyInit};
...
fn encrypt_x25519_chacha20_poly1305(
    ...
    let server_sk = SecretKey::try_from(&seckey[0..SecretKey::BYTES])
        .map_err(|_| Crypt4GHError::BadClientPrivateKey)?;
    let client_pk =
        PublicKey::try_from(recipient_pubkey).map_err(|_| Crypt4GHError::BadServerPublicKey)?;
    ...
    let shared_key =
        GenericArray::<u8, U32>::from_slice(&server_session_keys.rx.as_ref().as_slice());
    ...
    keys.iter()
        ...
        .map(|key| {
            match encrypt_x25519_chacha20_poly1305(packet, &key.privkey, &key.recipient_pubkey) {
                Ok(session_key) => {
                    Ok(vec![u32::from(key.method).to_le_bytes().to_vec(), session_key].concat())
                }
                Err(e) => Err(e),
            }
        })
...
pub fn serialize(packets: Vec<Vec<u8>>) -> Vec<u8> {
    ...
    vec![
        ...
        packets
            .into_iter()
            .flat_map(|packet| {
                vec![((packet.len() + 4) as u32).to_le_bytes().to_vec(), packet].concat()
            })
            .collect::<Vec<u8>>(),
    ]
    .concat()
...
fn decrypt_packet(
    ...
    let packet_encryption_method = bincode::deserialize::<u32>(packet)
        .map_err(|_| Crypt4GHError::ReadPacketEncryptionMethod)?;

    log::debug!(
        "Header Packet Encryption Method: {}",
        packet_encryption_method
    );
    ...
    for key in keys {
        ...
        match packet_encryption_method {
            0 => {
                let plaintext_packet =
                    decrypt_x25519_chacha20_poly1305(&packet[4..], &key.privkey, sender_pubkey);
...
fn decrypt_x25519_chacha20_poly1305(
    ...
    let client_sk = SecretKey::try_from(&privkey[0..SecretKey::BYTES])
        .map_err(|_| Crypt4GHError::BadClientPrivateKey)?;
    let server_pk =
        PublicKey::try_from(peer_pubkey).map_err(|_| Crypt4GHError::BadServerPublicKey)?;

    let keypair = Keypair::from(client_sk);
    let client_session_keys = keypair.session_keys_to(&server_pk);
    let shared_key =
        GenericArray::<u8, U32>::from_slice(&client_session_keys.tx.as_ref().as_slice());
    ...
    let plaintext = cipher.decrypt(&nonce, packet_data).map_err(|err| {
        Crypt4GHError::UnableToDecryptBlock(packet_data.to_vec(), err.to_string())
    })?;
...
fn partition_packets(packets: Vec<Vec<u8>>) -> Result<HeaderPackets, Crypt4GHError> {
    ...
    for packet in packets {
        let packet_type = bincode::deserialize::<HeaderPacketType>(&packet[0..4])
            .map_err(|_| Crypt4GHError::InvalidPacketType)?;
...
fn parse_enc_packet(packet: &[u8]) -> Result<Vec<u8>, Crypt4GHError> {
    match packet[0..4] {
        ...
        _ => Err(Crypt4GHError::UnsupportedEncryptionMethod(
            bincode::deserialize::<u32>(&packet[0..4])
                .expect("Unable to deserialize bulk encryption method"),
        )),
    }
}
...
fn parse_edit_list_packet(packet: &[u8]) -> Result<Vec<u64>, Crypt4GHError> {
    let nb_lengths: u32 =
        bincode::deserialize::<u32>(packet).map_err(|_| Crypt4GHError::NoEditListLength)?;
    ...
    (4..nb_lengths * 8)
        ...
        .map(|i| {
            bincode::deserialize::<u64>(&packet[i as usize..])
                .map_err(|_| Crypt4GHError::InvalidEditList)
        })
        .collect()
}
...
pub fn deconstruct_header_info(
    ...
    let header_info = bincode::deserialize::<HeaderInfo>(header_info_file)
        .map_err(|e| Crypt4GHError::ReadHeaderError(e))?;
...
pub fn rearrange<'a>(
    ...
    let final_packets = packets
        .into_iter()
        .map(|packet| {
            encrypt(&packet, &hash_keys).map(|encrypted_packets| encrypted_packets.concat())
        })
