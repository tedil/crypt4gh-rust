...
fn parse_range(arg: Option<String>) -> Result<(usize, Option<usize>), Crypt4GHError> {
    match arg {
        Some(range) => {
            ...
            let range_regex =
                Regex::new(r"(?P<start>[\d]+)-?(?P<end>[\d]+)?").expect("Bad range regex");
    ...
    if generate && seckey_path.is_none() {
        ...
        let passphrase: Result<String, Crypt4GHError> = match std::env::var(PASSPHRASE) {
            Ok(_) => {
                ...
                std::env::var(PASSPHRASE).map_err(|e| Crypt4GHError::NoPassphrase(e.into()))
                // TODO: Does this make sure it's set/unset?
            }
            Err(_) => prompt_password(format!("Passphrase for {:?}: ", path))
...
fn run_keygen(
    ...
    for key in &[seckey.to_owned(), pubkey.to_owned()] {
        ...
        if key.is_file() {
            ...
            remove_file(key)
                .unwrap_or_else(|_| panic!("Unable to remove key file (ERROR = {:?})", key));
        }
    }
...
fn run() -> Result<(), Crypt4GHError> {
    ...
    match matches.subcommand {
        ...
        Command::Decrypt {
            ...
            sender_pk,
            range,
        } => run_decrypt(sk, sender_pk, range)?,
        Command::Rearrange { sk, range } => run_rearrange(sk, range)?,
        Command::Reencrypt {
            sk,
            recipient_pk,
            trim,
        } => run_reencrypt(sk, &recipient_pk, trim)?,
        Command::Keygen {
            sk,
            pk,
            comment,
