...
fn load_from_pem(filepath: &PathBuf) -> Result<Vec<u8>, Crypt4GHError> {
    ...
    general_purpose::STANDARD
        ...
        .map_err(move |e| Crypt4GHError::BadBase64Error(e.into()))
}
...
fn derive_key(
    ...
    match alg {
        ...
        "bcrypt" => bcrypt_pbkdf::bcrypt_pbkdf(
            passphrase.as_bytes(),
            &salt.unwrap_or_else(|| {
                log::warn!("Using default salt = [0_u8; 8]");
                vec![0_u8; 0]
...
fn parse_c4gh_private_key(
    ...
    let ciphername = String::from_utf8(decode_string_c4gh(&mut stream)?)
        .map_err(|e| Crypt4GHError::BadCiphername(e.to_string()))?;
    ...
    let privkey_plain = ChaCha20Poly1305::new(key)
        .encrypt(nonce, encrypted_data)
...
fn parse_ssh_private_key(
    ...
    let kdfname = String::from_utf8(decode_string_ssh(&mut stream)?)
        .map_err(|e| Crypt4GHError::BadKdfName(e.into()))?;
    ...
    stream
        .read_exact(&mut buf)
        .map_err(|_| Crypt4GHError::ReadSSHKeys)?;
...
fn decipher(
    ...
    log::debug!(
        "Private ciphertext length and target ciphername block size: {} {}",
        private_ciphertext.len(),
        block_size(ciphername)?
    );

    if ((ivlen + keylen) as usize) != data.len() {
        return Err(Crypt4GHError::InvalidData(String::from(
            "IV length and Key length should match total data length",
        )));
    }

    if (private_ciphertext.len() % block_size(ciphername)?) != 0 {
        return Err(Crypt4GHError::InvalidData(String::from(
            "Ciphertext does not match target cipher block size",
        )));
    }
    ...
    match ciphername {
        "aes128-ctr" => {
            ...
            cipher
                .apply_keystream_b2b(&reader, writer.get_mut())
                .map_err(|_| Crypt4GHError::BadCiphername(String::from("aes128-ctr")))?
        }
        "aes192-ctr" => {
            ...
            cipher
                .apply_keystream_b2b(&reader, writer.get_mut())
                .map_err(|_| Crypt4GHError::BadCiphername(String::from("aes192-ctr")))?
        }
        "aes256-ctr" => {
            ...
            cipher
                .apply_keystream_b2b(&reader, &mut writer.get_mut())
                .map_err(|_| Crypt4GHError::BadCiphername(String::from("aes256-ctr")))?
        }
        "aes128-cbc" => {
            todo!();
        }
    ...
    writer
        .into_inner()
        .map_err(|err| Crypt4GHError::IoError(err.into_error()))
}
...
fn get_skpk_from_decrypted_private_blob(
    blob: &[u8],
) -> Result<([u8; 32], [u8; 32]), Crypt4GHError> {
    let check_number_1: u32 =
        bincode::deserialize(&blob[0..4]).map_err(|_| Crypt4GHError::ReadCheckNumber1Error)?;
    let check_number_2: u32 =
        bincode::deserialize(&blob[4..8]).map_err(|_| Crypt4GHError::ReadCheckNumber2Error)?;
    assert!(
    ...
    assert!(
        skpk.len() == 64,
        "The length of the private key blob must be 64"
    );
...
fn ssh_get_public_key(line: &str) -> Result<[u8; 32], Crypt4GHError> {
    ...
    let pkey = general_purpose::STANDARD
        .decode(
            line[12..]
                .split(' ')
                .take(1)
...
pub fn generate_keys(
    ...
    pk_file
        .write_all(b"-----BEGIN CRYPT4GH PUBLIC KEY-----\n")
        .unwrap();
    pk_file
        .write_all(general_purpose::STANDARD.encode(pk).as_bytes())
        .unwrap();
    pk_file
    ...
    sk_file
        .write_all(b"-----BEGIN CRYPT4GH PRIVATE KEY-----\n")
        .unwrap();
    sk_file
        .write_all(general_purpose::STANDARD.encode(sk_encrypted).as_bytes())
        .unwrap();
    sk_file
...
fn encode_private_key(
    ...
    Ok(if passphrase.is_empty() {
        ...
        vec![
            ...
            encode_string_c4gh(Some(
                &vec![(rounds as u32).to_be_bytes().to_vec(), salt.to_vec()].concat(),
            )),
            encode_string_c4gh(Some(b"chacha20_poly1305")),
            encode_string_c4gh(Some(
                &vec![nonce_array.to_vec(), encrypted_key_ga.to_vec()].concat(),
            )),
            match comment {
                Some(c) => encode_string_c4gh(Some(c.as_bytes())),
                None => [].to_vec(),
